...
onMounted(async () => {
  try {
    const perfil = await axios.get('http://34.51.158.117:3000/api/perfil', {
      headers: { Authorization: `Bearer ${token}` }
    });
    user.value = perfil.data;

    const res = await axios.get('http://34.51.158.117:3000/api/utilizadores');
...
async function atualizarRole(utilizador) {
  try {
    await axios.put(`http://34.51.158.117:3000/api/utilizadores/${utilizador._id}`, {
